    ...
    in
    {
      ...
      checks = forAllSystems (system:
        let pkgs = nixpkgs.legacyPackages.${system}; in
        {
          ...
          # The two roles this profile fills with a package and nothing else -- synthetic typing
          # and input automation. A real `nixosSystem` again, but for the opposite reason to the
          # two above: not because an option half could be forgotten, but because the package list
          # a real evaluation ends up with is the whole claim -- and, for input automation, because
          # the thing that has to stay false is one of nixpkgs' OWN options, which no stub could
          # report on.
